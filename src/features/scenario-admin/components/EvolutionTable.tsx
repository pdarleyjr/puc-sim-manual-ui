...
export function EvolutionTable({ scenarioId, evolutions }: Props) {
  const { addEvolution, updateEvolution, deleteEvolution } = useScenarioAdmin();
      ...
      <div className="border border-gray-200 rounded-lg overflow-hidden">
        <table className="w-full">
          ...
          <tbody className="divide-y divide-gray-200">
            {evolutions.map((evo) => (
              <tr key={evo.id} className="hover:bg-gray-50">
                ...
                <td className="px-4 py-3 text-right">
                  <button
                    onClick={() => deleteEvolution(scenarioId, evo.id)}
                    ...
                    title={evolutions.length === 1 ? 'Cannot delete last evolution' : 'Delete'}
                  >
